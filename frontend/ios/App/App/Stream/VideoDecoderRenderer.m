...
// NOTE (App 統合 ARK 2026-05-06):
// 公式 moonlight-ios の VideoDecoderRenderer は StreamView (公式 UI 用 UIView サブクラス) を
// 受け取る前提だったが、 App は独自 UI に統合するので汎用 UIView をそのまま使う。
// `_view` の型を `UIView*` にして StreamView import を外した。 公式コードからの差分はこの 1 点のみ。
...
- (int)submitDecodeBuffer:(unsigned char *)data length:(int)length bufferType:(int)bufferType decodeUnit:(PDECODE_UNIT)du
{
    ...
    if (formatDesc == NULL) {
        // Can't decode if we app't gotten our parameter sets yet
        free(data);
        return DR_NEED_IDR;
    }
    ...
    // moonlight-common-c の新 API で presentationTimeMs → presentationTimeUs (μs) に変わったので
    // CMTimeMake の timescale を 1000 → 1000000 に変える。
