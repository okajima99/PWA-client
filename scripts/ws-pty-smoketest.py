...
そのまま stdout に流す。 penalty regression 確認 (= docs/penalty-baseline.md) の
前段として手動で動作確認したい時に使う。

使い方:
    # backend 起動済 + config.json に "use_pty_runner": true を入れた状態で:
...
penalty 確認のフロー:
    1. `python3 scripts/penalty-bench.py <log>` で baseline 確認
    2. 本 script で 1 turn (= 固定 prompt 送って response 待つ)
    3. もう一度 bench で delta 確認
"""
